...
if __name__ == "__main__":
    ...
    parser.add_argument("--steps", type=int, default=3000, help="optimize iterations")
    ...
    for duplicate in range(1):
        ...
        for i in pbar:
            ...
            # #-----NF ---------------------------------------------------------------------------
            mse_loss = F.l1_loss(Downsampler(img_gen), imgs)
            ...
            loss = 10 * mse_loss + args.logp * logp_loss + args.pnorm * p_norm_loss
            cross_loss = 0
            if args.w_plus:
            ...
            if loss < min_loss:
                min_loss = loss
                best_summary = f'L2: {mse_loss.item():.4f}; cross: {cross_loss:.4f};' \
                               f'logp: {logp_loss}; pn: {p_norm_loss.item():.4f},'
        ...
        if best_mse > 0.2:
            ...
            for i, input_name in enumerate(args.files):
                ...
                img_name = os.path.splitext(os.path.basename(input_name))[0] + 'lr' + str(args.lr).split('.')[-1] \
                           ...
                           + '_pnorm' + str(args.pnorm).split('.')[-1] + '_step' + str(best_step) \
                           + 'num' + str(duplicate) + '_n' + str(num_trainable_noise_layers) + '_0.png'
