...
def process(n_tol, sigma):
    ...
    data_x, data_y = [], []
    n_c = 1000
    input_dir_HR = 'input/superres/HR_Golgi/'
    size = 128
    ...
    #         data_y.append(label)
    data_x = load_file(os.path.join('summary', f'HR{0}.csv')) + load_file(os.path.join('summary', f'HR{1}.csv'))
    data_y = [0] * n_c + [1] * n_c
    X_train = np.expand_dims(np.stack(data_x, 0), axis=1)
    ...
    # input_dir_LR = 'input/superres/LR_BBBC021_16x/'
    # input_dir_SR = 'input/superres/SR_PULSE_Golgi_16x/'
    # for label in range(2):
    ...
    # data_x_lr = load_file(os.path.join('summary', f'SR_PULSE{0}.csv')) + load_file(
    #     os.path.join('summary', f'SR_PULSE{1}.csv'))
    data_x_lr = load_file(os.path.join('summary', f'PULSE_16x{0}.csv')) + load_file(
        os.path.join('summary', f'PULSE_16x{1}.csv'))
    data_y_lr = [0] * n_c + [1] * n_c
    X_test_lr = np.expand_dims(np.stack(data_x_lr, 0), axis=1)
