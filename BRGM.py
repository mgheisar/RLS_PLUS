...
import dnnlib
import torch_utils_brgm.legacy
...
def project(
    ...
    for step in range(num_steps):
        ...
        loss, pixelwise_loss, perceptual_loss, w_loss, cosine_loss, synth_images, synth_images_down = optimizerAdam.step(
            closure=closure)
...
@click.command()
@click.option('--network_pkl', default="checkpoint/ffhq.pkl", help='Network pickle filename')
@click.option('--inputdir', default="input/project/input", help='Folder of input (target) images to project to',
              metavar='FILE')
...
@click.option('--save-video', help='Save an mp4 video of optimization progress', type=bool, default=False,
              show_default=False)
@click.option('--outdir', default='input/project/input/out', help='Where to save the output images', required=True,
              metavar='DIR')
@click.option('--recontype', default="super-resolution",
              help='Type of reconstruction: "none" (normal image inversion), "super-resolution", "inpaint" ',
              required=True)
@click.option('--superres-factor', default=64, help='Super-resolution factor: 2,4,8,16,32,64,128,256 ', type=int)
@click.option('--masks',
...
def run_projection(
  ...
  """
    ...
    with open(network_pkl, 'rb') as fp:
        G = torch_utils_brgm.legacy.load_network_pkl(fp)['G_ema'].requires_grad_(False).to(device)  # type: ignore
