...
if __name__ == "__main__":
    ...
    logp_vec = [0, 0.0005, 0.001, 0.005, 0.01]
    cross_vec = [0, 0.01, 0.05, 0.1, 0.5]
    pnorm_vec = [0, 0.01, 0.05, 0.1, 0.5]
    lr_vec = [0.1, 0.5]
    for i_lr in range(len(lr_vec)):
        args.lr = lr_vec[i_lr]
        for i_log in range(len(logp_vec)):
            args.logp = logp_vec[i_log]
            for i_cr in range(len(cross_vec)):
                args.cross = cross_vec[i_cr]
                for i_pn in range(len(pnorm_vec)):
                    args.pnorm = pnorm_vec[i_pn]
                    if args.w_plus:
