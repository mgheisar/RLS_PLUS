...
#     )
#     parser.add_argument('--input_dir', type=str, default='input/project/resHR', help='input data directory')
#     parser.add_argument('--output_dir', type=str, default='input/project/resLR_128x', help='output data directory')
#     parser.add_argument("--size", type=int, default=1024, help="output HR image size")
#     parser.add_argument("--scale", type=int, default=128, help="Super resolution scale")
#     parser.add_argument("--exp", type=str, default="FFHQ", help="face or cells")
...
#     pilimg.save(Path(f"{output_dir}") / (im.stem.split('_')[0] + f"_{scale}_LR.jpg"))

import glob
scale = 8
output_dir = f"input/project/resSR/resbic_{scale}x"
img_list = glob.glob(f"input/project/resLR_{scale}x/*.jpg")
for img in img_list:
    im = Path(img)
    img = Image.open(im)
    pilimg = img.resize((1024, 1024), Image.BICUBIC)
    pilimg.save(Path(f"{output_dir}") / (im.stem.split('_')[0] + f"_{scale}.jpg"))
