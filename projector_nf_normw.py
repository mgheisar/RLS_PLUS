...
if __name__ == "__main__":
    ...
    if args.w_plus:
        # latent_mean = torch.load("w_pulse_nf_0", map_location=map_location)
    ...
    for i in pbar:
        t = i / args.steps
        optimizer.zero_grad()
        # latent_in = latent_in * gaussian_fit["std"] + gaussian_fit["mean"]
        ...
        # x = (x * torch.from_numpy(data["std"]).to(x)) + torch.from_numpy(data["mu"]).to(x)
        cc = torch.mean(torch.norm(x, dim=-1))
        ...
        loss = 5 * p_loss + 1 * mse_loss + 0.005 * p_norm_loss + 0.00001 * logp_loss
        cross_loss = 0
        if args.w_plus:
    ...
    torch.save(best_latent, "w_nfnormw")
    torch.save(noises, "w_nfnormw_n")
    result_file = {}
    for i, input_name in enumerate(args.files):
        ...
        img_name = os.path.splitext(os.path.basename(input_name))[0] + f'nfnormw_w{args.w_plus}005_00001.png'
