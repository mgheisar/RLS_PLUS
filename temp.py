...
from sr_boost_ada import noise_normalize_, project_onto_l1_ball
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_trainable_noise_layers_ada", type=int, default=9, help="number of trainable noise layers")
    parser.add_argument("--radius_ada", type=int, default=1, help="radius of the l1 ball")
    parser.add_argument("--steps_ada", type=int, default=200, help="optimize iterations")
    parser.add_argument("--lr_ada", type=float, default=0.1, help="learning rate")
    # ---------------------------------------------------
    ...
    parser.add_argument("--num_trainable_noise_layers", type=int, default=0,
                        help="number of trainable noise layers")
    ...
    if args.clas is None:
        image_list = sorted(glob.glob(f"{args.input_dir}/*_{args.factor}x.jpg"))[:2000]
        # image_list = sorted(glob.glob(f"{args.input_dir}/*.jpg"))[:1000]
    else:
    ...
    for ref_im, ref_im_hr, ref_im_name in dataloader:
        ...
        desc = "logp" + str(args.logp) + "_pnorm" + str(args.pnorm) + "_cross" + str(args.cross)
        # img_name = f'{ref_im_name[0]}_boost0_{desc}.jpg'
        # pil_img.save(f'{args.out_dir}/{img_name}')
        #  ### Second stage------------------------------------------------------------
        w_anchor = best_latent
        latent_in = w_anchor.detach().clone()
        noises = []  # stores all of the noise tensors
        noise_vars = []  # stores the noise tensors that we want to optimize on
        num_trainable_noise_layers = args.num_trainable_noise_layers_ada  # number of noise layers that we want to optimize on
        ...
        optimizer = torch.optim.Adam(var_list, lr=args.lr_ada)
        optimizer_g = torch.optim.Adam(g_ema.parameters(), lr=0.0001)
        schedule_dict = {
            'fixed': lambda x: 1,
            'linear1cycle': lambda x: (9 * (1 - np.abs(x / args.steps_ada - 1 / 2) * 2) + 1) / 10,
            'linear1cycledrop': lambda x: (9 * (
                    1 - np.abs(
                x / (0.9 * args.steps_ada) - 1 / 2) * 2) + 1) / 10 if x < 0.9 * args.steps_ada else 1 / 10 + (
                    x - 0.9 * args.steps_ada) / (0.1 * args.steps_ada) * (1 / 1000 - 1 / 10),
        }
        schedule_func = schedule_dict[args.lr_schedule]
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, schedule_func)
        pbar = tqdm(range(args.steps_ada))
        min_loss = np.inf
        ...
        for i in pbar:
            ...
            if loss < min_loss:
                ...
                best_step = i + 1
                best_rec = l1_loss.item()
            # if i == 0 or i == args.steps_ada-21 or i == args.steps_ada-11 or i == args.steps_ada-1:
            if i == args.steps_ada - 1:
                im = img_gen.detach().clone()
                pil_img = toPIL(im[0].cpu().detach().clamp(0, 1))
                desc = "logp" + str(args.logp) + "_pnorm" + str(args.pnorm) + "_cross" + str(args.cross)
                img_name = f'{ref_im_name[0]}_boost{i}_{desc}.jpg'
                pil_img.save(f'{args.out_dir}/{img_name}')
            ...
            #  #------ w+= True
            deviations = [project_onto_l1_ball(lat_in - lat_m, args.radius_ada * np.sqrt(512))
