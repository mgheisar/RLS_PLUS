...
if __name__ == "__main__":
    ...
    parser.add_argument("--steps", type=int, default=50000, help="optimize iterations")
    parser.add_argument(
        "--noise_regularize",
        type=float,
        default=0,
    ...
    # if args.w_plus:
    #     latent = torch.randn(
    #         (imgs.shape[0], g_ema.n_latent, 512), dtype=torch.float, requires_grad=True, device=device)
    # else:
    #     latent = torch.randn((imgs.shape[0], 512), dtype=torch.float, requires_grad=True, device=device)
    # latent_in = flow.forward(latent, None, mode='inverse')[0]
    ...
    # x = cvt(x)
    # logp = flow.logp(x)
    # print("logp latent: ", -torch.mean(logp))
    x = latent_in.view(-1, latent_in.size(-1))
    x = torch.nn.LeakyReLU(5)(x)
    x = (x - torch.from_numpy(data["mu"]).to(x)) / torch.from_numpy(data["std"]).to(x)
    ...
    for i in pbar:
        ...
        loss = 0.75 * p_loss + 1 * mse_loss + 0.00005 * logp_loss
        cross_loss = 0
        if args.w_plus:
    ...
    # torch.save(best_latent, "w_nf_spher")
    # torch.save(noises, "w_nf_spher_n")
    # result_file = {}
    ...
    #     }
    #
    #     img_name = os.path.splitext(os.path.basename(input_name))[0] + f'nf_spher_{args.w_plus}.png'
