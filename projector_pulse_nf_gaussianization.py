...
if __name__ == "__main__":
    ...
    #     latent = latent.unsqueeze(1).repeat(1, g_ema.n_latent, 1)
    # else:
    ...
    if args.w_plus:
        ...
        latent = latent.unsqueeze(1).repeat(1, g_ema.n_latent, 1).detach().clone()
    else:
        latent = torch.randn((imgs.shape[0], 512), dtype=torch.float32, device=device).detach().clone()
    latent.requires_grad = True
    opt_dict = {
        'sgd': torch.optim.SGD,
    ...
    print("logp latent: ", logp.item())

    for i in pbar:
        t = i / args.steps
        optimizer.opt.zero_grad()
        latent_in = latent
        # ln = torch.mean(torch.norm(latent, dim=-1))
        latent_in = flow.forward(latent_in, None, mode='inverse')[0]
        latent_in = (latent_in * torch.from_numpy(data["std"]).to(latent_in)) + torch.from_numpy(data["mu"]).to(latent_in)
        ...
        if loss < min_loss:
            ...
            best_summary = f'Percept: {p_loss.item():.4f};L1: {mse_loss.item():.4f};' \
                           f'logp_loss: {logp_loss}; p norm: {p_norm_loss.item():.4f},'
    ...
    torch.save(best_latent, "w_pulseg_")
    torch.save(noises, "w_pulseg_n")
    result_file = {}
    for i, input_name in enumerate(args.files):
        ...
        img_name = os.path.splitext(os.path.basename(input_name))[0] + f'nfg_w{args.w_plus}_.png'
